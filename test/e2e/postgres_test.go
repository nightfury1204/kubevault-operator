...
var _ = Describe("Postgres role and role binding", func() {
	...
	var (
		...
		IsPostgresRoleCreated = func(name, namespace string) {
			...
			Eventually(func() bool {
				_, err := f.DBClient.EngineV1alpha1().PostgresRoles(namespace).Get(name, metav1.GetOptions{})
				return err == nil
	...
	Describe("PostgresRole", func() {
		...
		Context("Create PostgresRole", func() {
			...
			AfterEach(func() {
				err := f.DBClient.EngineV1alpha1().PostgresRoles(p.Namespace).Delete(p.Name, &metav1.DeleteOptions{})
			...
			It("should be successful", func() {
				_, err := f.DBClient.EngineV1alpha1().PostgresRoles(pgRole.Namespace).Create(&p)
	...
	Describe("DatabaseAccessRequest", func() {
		...
		Context("Create, Approve, Deny DatabaseAccessRequest", func() {
			BeforeEach(func() {
				_, err := f.DBClient.EngineV1alpha1().PostgresRoles(pRole.Namespace).Create(&pRole)
				Expect(err).NotTo(HaveOccurred(), "Create PostgresRole")
			...
			AfterEach(func() {
				...
				err = f.DBClient.EngineV1alpha1().PostgresRoles(pRole.Namespace).Delete(pRole.Name, &metav1.DeleteOptions{})
				Expect(err).NotTo(HaveOccurred(), "Delete PostgresRole")
